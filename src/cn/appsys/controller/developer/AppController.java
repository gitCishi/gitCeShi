		...
		if(!attach.isEmpty()){
			...
			if(attach.getSize() > filesize){//上传大小不得超过 50k
			   ...
			   ||prefix.equalsIgnoreCase("jepg") || prefix.equalsIgnoreCase("pneg")){//上传图片格式
            	...
            	System.out.println("fileName:"+fileName+"path"+fileName);
				 File targetFile = new File(path,fileName);
				 System.out.println("tar"+targetFile);
				 if(!targetFile.exists()){
					 System.out.println("tar"+targetFile);
					 targetFile.mkdirs();
					 System.out.print("targ"+targetFile);
				 }
				 try {
					attach.transferTo(targetFile);
				} catch (Exception e) {
				 ...
				 logoPicPath = request.getContextPath()+"/statics/uploadfiles/"+fileName;
				 logoLocPath = path+File.separator+fileName;
				 System.out.println(logoLocPath+"aaa");
				 System.out.println("pic"+logoPicPath+"\nre:"+request.getContextPath());
            }else{
            	return "redirect:/dev/flatform/app/appinfomodify?id="+appInfo.getId()
						 +"&error=error3";
            }
